		<div class="main">
			<form method="POST" action="GestionEquipeJoueur.php" name="formEquipeJoueur" id="formEquipeJoueur"
				...
				<div class='blocLeft'>
					...
					<div class='liens'>
						{if $profile <= 8 && $Verrou != 'O' && $AuthModif == 'O' && $idEquipe > 0}
							<a href="#" onclick="setCheckboxes('formEquipeJoueur', 'checkEquipeJoueur', true);return false;"><img
									height="22" src="../img/glyphicons-155-more-checked.png" title="{#Tous#}" /></a>
							<a href="#" onclick="setCheckboxes('formEquipeJoueur', 'checkEquipeJoueur', false);return false;"><img
									height="22" src="../img/glyphicons-155-more-windows.png" title="{#Aucun#}" /></a>
							<a href="#" onclick="RemoveCheckboxes('formEquipeJoueur', 'checkEquipeJoueur')"><img height="25"
									src="../img/glyphicons-17-bin.png" title="{#Supprimer#}" /></a>
						{/if}
						<a href="FeuillePresence.php?equipe={$idEquipe}" target="_blank"
							title="{#Feuille_de_presence#} (PDF) (FR)"><img height="25" src="../img/pdf.png" /></a>
						...
						<select name='changeEquipe' id='changeEquipe'>
							...
							{section name=i loop=$arrayEquipe}
								<Option Value="{$arrayEquipe[i].Id}" {if $idEquipe == $arrayEquipe[i].Id}selected{/if}>
									{$arrayEquipe[i].Code_comite_dep} - {$arrayEquipe[i].Libelle}</Option>
							{/section}
						</select>
					...
					<div class='blocTable'>
								...
								{section name=i loop=$arrayJoueur}
									...
									<tr class='{cycle values="impair,pair"}  colorCap{$arrayJoueur[i].Capitaine}'>
										{if $profile <= 7 && $Verrou != 'O' && $AuthModif == 'O' && $idEquipe > 0}
											<td><input type="checkbox" name="checkEquipeJoueur" value="{$arrayJoueur[i].Matric}"
													id="checkDelete{$smarty.section.i.iteration}" /></td>
											<td width="30" class='directInput text' tabindex='1{$smarty.section.i.iteration}0'><span href="#"
													Id="Numero-{$arrayJoueur[i].Matric}-{$idEquipe}">{$arrayJoueur[i].Numero}</span></td>
										...
										<td>
											...
											{if $profile <= 6 && $AuthModif == 'O'}
												<a href="GestionAthlete.php?Athlete={$arrayJoueur[i].Matric}"><img width="10" src="../img/b_plus.png"
														title="{#Details#}" /></a>
											{/if}
										</td>
										<td>{$arrayJoueur[i].Numero_club}</td>
										<td>{$arrayJoueur[i].Categ}
											{if $arrayJoueur[i].date_surclassement != ''}<span class="vert"
												title="Surclassement au {$arrayJoueur[i].date_surclassement}">#S</span>{/if}
										...
										<td>{if $arrayJoueur[i].CertifCK != 'OUI'}<span class='highlight2'
												title='{#Competition#}'>{#NON#}</span>{else}<span title='{#Competition#}'>{#OUI#}</span>
											{/if}</td>
										<td>{$arrayJoueur[i].Arbitre}</td>
										{if $profile <= 7 && $Verrou != 'O' && $AuthModif == 'O'}
											<td><a href="#"
													onclick="RemoveCheckbox('formEquipeJoueur', '{$arrayJoueur[i].Matric}');return false;"><img
						...
						<div>
							...
							<br>
							<a href="FeuillePresence.php?equipe={$idEquipe}" target="_blank"
								title="{#Feuille_de_presence#} (PDF) (FR)"><img height="25" src="../img/pdf.png" />{#Feuille_de_presence#}
								(PDF) (FR)</a>
							...
							{else}
								<a href="FeuillePresenceEN.php?equipe={$idEquipe}" target="_blank"
									title="{#Feuille_de_presence#} (PDF) (EN)"><img height="25"
										src="../img/pdfEN.png" />{#Feuille_de_presence#} (PDF) (EN)</a>
							{/if}
						</div>
					</div>
						...
						<table width="100%">
							...
							<tr>
								<td colspan=2 align="center">
									<span id="irregularite" class='highlight2'>Ce joueur n'est pas en règle<br />pour une compétition
									nationale</span>
								<br />
								<span id="motif" class='highlight2'></span>
								<input type="button" id="addEquipeJoueurImpossible" value="Ajout impossible !">
								<br />
								<input type="button" onclick="Add2();" name="addEquipeJoueur2" id="addEquipeJoueur2"
									value="<< {#Ajouter#}">
								{if $profile < 3}
								<br />
								<br />
...
-->
						...
						<tr>
							<td>
								...
								<input type="text" name="naissanceJoueur" maxlength=10 id="naissanceJoueur"
									onfocus="displayCalendar(document.forms[0].naissanceJoueur,'dd/mm/yyyy',this)">
							</td>
							<td>
								<label for="sexeJoueur">{#Sexe#} :</label>
								<input type='radio' id='sexeJoueur_M' name='sexeJoueur' value='M' checked />M
								<input type='radio' id='sexeJoueur_F' name='sexeJoueur' value='F' />F
							</td>
						</tr>
						<tr>
							<td colspan=2>

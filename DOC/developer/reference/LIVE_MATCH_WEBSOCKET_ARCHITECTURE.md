# Architecture temps réel d'un match (Scorepad → WSM → KPI → Live)
...
Le mode **complet Bodet** est décrit en priorité ; les modes **alternatifs et dégradés** (feuille
...
> ⚠️ Ce document décrit d'abord le fonctionnement **complet** avec matériel Bodet (Scorepad +
> Boxpro). En pratique, **tous les terrains n'ont pas ce matériel** : les modes **alternatifs et
...
flowchart LR
    ...
    SP -->|série ASCII Bodet 9600 bauds<br/>RJ45 ou pont wifi| BXP
...
**Idée directrice** : le Scorepad Bodet est la **source de vérité** de la saisie live. La Boxpro
...
### 2.1 Scorepad Bodet (hardware, table de marque)
...
**Sortie protocole vers la Boxpro** : le Scorepad expose une **sortie série RJ45** (RS-232,
9600 bauds, 8 bits + start + stop, sans parité) transmettant des **trames ASCII** propriétaires
Bodet (trames de la forme `SOH · adresse · STX · CTRL · Message · ETX · LRC`). Le détail des trames
est **hors périmètre** de ce document. C'est ce flux que la Boxpro décode puis convertit en
WebSocket STOMP.

> **Raccordement réseau Scorepad ↔ Boxpro** : la liaison se fait au sein du **réseau local**, soit
...
### 2.2 Boxpro (passerelle)
Boîtier sur le réseau local. Reçoit les trames **série ASCII Bodet (9600 bauds)** du Scorepad — via
**RJ45 ou pont wifi** sur le LAN — les **décode**, puis les **convertit en WebSocket STOMP** (même
réseau local). C'est elle qui expose l'URL `ws://<ip-boxpro>:<port>` à laquelle `app_wsm` se connecte.
...
Application Vue 3 exploitée près du terrain, sur le même réseau local que la Boxpro.
C'est le **cœur** du système :
- se **connecte en STOMP** aux flux de la Boxpro (un par terrain) ;
...
```mermaid
sequenceDiagram
    participant SP as Scorepad Bodet
    participant BXP as Boxpro
    participant WSM as app_wsm
    participant DB as KPI (DB via /api/wsm)
    participant BRK as Broker KPI
    participant LIVE as app_live

    Note over SP,BXP: série ASCII Bodet (RJ45/pont wifi) puis STOMP (LAN)
    SP->>BXP: trames série ASCII Bodet (9600 bauds, RJ45 ou pont wifi)
    ...
    Note over WSM,BXP: WSM peut aussi PUSHER vers la Boxpro
    WSM->>BXP: STOMP /api/game/set-teams (compositions depuis KPI)
    WSM->>BXP: STOMP /api/game/sync (demande de resync)
```

Deux sens coexistent sur le lien STOMP WSM ↔ Boxpro :

| Sens | Destination STOMP | Contenu | Déclencheur |
|------|-------------------|---------|-------------|
| **Boxpro → WSM** (abonnements) | `/game/chrono`, `/game/data-game`, `/game/period`, `/game/player-info`, `/game/game-state`, `/game/team-game`, `/game/set-teams`, `/game/ready-to-start-game`, `/game/game-phase` | état live saisi sur le Scorepad | saisie table de marque |
| **WSM → Boxpro** (publications) | `/api/game/set-teams`, `/api/game/sync` | compositions d'équipes (issues de KPI), demande de resynchro | chargement d'un match, ready-to-start |

Sur `ready-to-start-game`, WSM charge le match suivant depuis KPI (`fetchGame`) et **pousse la
composition** (`set-teams` : noms, couleurs, maillots, logos base64, coachs) vers la Boxpro, qui
peut ainsi préremplir le Scorepad.

---

## 4. Détail des protocoles

### 4.1 STOMP (Boxpro ↔ app_wsm)
...
Codes cartons (Bodet → KPI) : `GREEN→V`, `YELLOW→J`, `YELLOW_RED/RED→R`, `RED_EJECTION→D`.
...
| `1` … `19` | **Terrains** (`pitch` / `numero_terrain`), un flux Boxpro STOMP chacun |
...
|---------|--------|--------|-----------|
| Scorepad Bodet | saisie manuelle | scoreboard/satellites (filaire/radio), HDMI, série ASCII Bodet vers Boxpro | non |
| Boxpro | série ASCII Bodet 9600 bauds (RJ45/pont wifi) | STOMP (LAN) | non |
| app_wsm | STOMP (Boxpro) | fetch DB + WS broker | IndexedDB local |
...
## 9. Référence des messages STOMP (Boxpro → app_wsm)
...
(méthode `fake()`), qui reproduit fidèlement le format émis par la Boxpro. Chaque message est un
...
|-------------------|------|----------------|
| `/game/ready-to-start-game` | Boxpro → WSM | charge match + push `set-teams` |
| `/game/set-teams` | Boxpro → WSM | passe le match à `ON` |
| `/game/game-state` | Boxpro → WSM | clôture de match |
| `/game/period` | Boxpro → WSM | période |
| `/game/chrono` | Boxpro → WSM | chrono de jeu + shotclock |
| `/game/data-game` | Boxpro → WSM | score + pénalités équipe |
| `/game/player-info` | Boxpro → WSM | but / carton joueur |
| `/game/team-game` | Boxpro → WSM | joueurs actifs |
| `/game/game-phase` | Boxpro → WSM | ignoré (log) |
| `/api/game/set-teams` | WSM → Boxpro | pousse les compositions KPI |
| `/api/game/sync` | WSM → Boxpro | demande de resynchronisation |

---

## 10. Démarrage & enchaînement des matchs (WSM ↔ Boxpro)

C'est l'étape **la plus sensible** du système : elle charge la bonne composition d'équipes dans le
Scorepad au bon moment, et détermine sur quel match les événements suivants seront imputés. Un
`set-teams` sur le mauvais match écrit les buts/cartons sur la mauvaise fiche en base.

### 10.1 Message sortant `/api/game/set-teams` (WSM → Boxpro)
...
Utile si le Scorepad n'a pas émis `ready-to-start`, si la mauvaise composition a été poussée, ou
...
| **Sync** | `syncRequest(n)` | réinitialise l'état local et demande un resync à la Boxpro |
...
Toutes les données reçues de la Boxpro **ne sont pas égales** : certaines sont écrites en base KPI
...
- Les **incrustations PHP** fonctionnent **entièrement sur le cache**, sans broker WebSocket : elles
  restent à jour tant que le worker régénère les fichiers, même si aucune Boxpro/WSM n'est active.
...
Le fonctionnement décrit dans les §1–13 est le mode **complet Bodet** : Scorepad + Boxpro sur le
terrain, `app_wsm` qui persiste et rediffuse. Mais sur un événement réel, **tous les terrains ne
disposent pas** de ce matériel (pas de Scorepad, ou pas de connexion Boxpro pour certains terrains).
...
|------|--------|--------------|------------------------|----------------------|
| **A — Complet Bodet** (§1–13) | Scorepad → Boxpro (STOMP) → `app_wsm` | `fetch` `/api/wsm/*` | oui (WSM → broker) | complète (score, chrono, shotclock, événements) |
...
C'est donc un mode **quasi complet sans matériel Bodet** : l'opérateur remplace le Scorepad, KPI
remplace la Boxpro/WSM pour la partie diffusion. L'incrustation reçoit à la fois le **live broker**
et le **cache**.
...
flowchart LR
    subgraph modes["Qui alimente la base / le broker ?"]
        A["Mode A<br/>Scorepad+Boxpro → app_wsm"]
...
Si un terrain n'a **ni matériel Bodet ni saisie live** (feuille de marque remplie plus tard, ou
...
Utile pour un terrain sans Scorepad où un opérateur veut au moins diffuser le score en direct sans

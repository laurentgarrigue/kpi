...
L'architecture actuelle (mode complet Bodet) fonctionne mais est **trop complexe et trop
...
1. **Dépendance à un poste + navigateur ouverts** — `app_wsm` (le WebSocket Manager) est une
   ...
   durée** de l'événement (2–5 jours) pour faire le relais Boxpro STOMP → base + broker. Si
...
4. **État du match dispersé** — la vérité live vit à la fois dans le Scorepad (hardware), dans
...
5. **Modes multiples mal unifiés** — Bodet complet, Feuille de marque V2 (base seule), V3
...
relais Boxpro** et **comment on garantit l'état**.
...
1. **Transport** : connexion STOMP à la Boxpro (LAN terrain).
2. **Traduction** : trames Bodet → modèle KPI.
...
Contrainte physique à garder en tête : la Boxpro n'est joignable que sur le **LAN du terrain**.
Donc le composant qui parle STOMP à la Boxpro doit être **sur ce LAN** (ou atteindre la Boxpro
...
| Option | Idée en une phrase | Où vit le relais Boxpro | Effort |
|--------|--------------------|--------------------------|--------|
| **1 — Relais serveur (broker STOMP côté KPI)** | La Boxpro pousse vers un service serveur ; plus rien dans le navigateur. | VPS KPI (si Boxpro atteignable) | ★★★★☆ |
...
l'Event Worker : process PHP/Node en boucle, supervisé) se connecte aux flux Boxpro et écrit
directement dans la base via `ScoringController`. Plus aucun navigateur dans la chaîne de
persistance.

Deux variantes selon l'atteignabilité de la Boxpro :

- **1a — Boxpro exposée** : la Boxpro (ou un port-forward/VPN sur le site) est joignable depuis le
  VPS ; le worker s'y connecte en STOMP sortant.
- **1b — Boxpro pousse (webhook/WS inversé)** : la Boxpro (ou un mini-relais sur site) initie la
...
```mermaid
flowchart LR
    SP["Scorepad"] -->|série| BXP["Boxpro (LAN terrain)"]
...
- **Point dur réseau** : exige que la Boxpro soit atteignable depuis Internet (1a) — souvent
  **impossible** sur un gymnase/site sportif (NAT, Wi-Fi captif, pas d'admin réseau). 1b lève ce
  point mais **suppose de pouvoir configurer la Boxpro** pour émettre vers le serveur, ce qui
  dépend du firmware Bodet (**à vérifier — risque bloquant**).
...
### 4.4 Complexité : ★★★★☆
Faisabilité **conditionnée** à l'atteignabilité Boxpro. Si acquise, l'implémentation est modérée
...
terrains, sur le **LAN**. Il remplace l'onglet `app_wsm` : il tient les connexions STOMP Boxpro,
...
> **Cas d'usage visé : réseau du site non maîtrisé, Boxpro non redirigeable.** C'est précisément
> quand on **ne peut pas** rediriger un port vers la Boxpro depuis Internet (option 1 impossible)
> que l'agent est la seule voie : il **initie une connexion sortante** vers KPI (traverse le NAT
> **sans** config réseau du site) tout en restant **sur le LAN** côté Boxpro — exactement ce que
> `app_wsm` sait déjà faire, mais en **service headless** au lieu d'un onglet.

```mermaid
flowchart LR
    subgraph site["Site — LAN (mini-PC/RPi)"]
        BXP1["Boxpro T1"] --> AG["Agent headless<br/>(daemon supervisé)<br/>+ buffer local SQLite"]
        BXP2["Boxpro T2"] --> AG
    end
...
Boxpro↔KPI ; **tout le reste des rôles de `app_wsm` est déporté** sur le worker serveur et app4.

| Rôle actuel de `app_wsm` | Où il va | Pourquoi |
|--------------------------|----------|----------|
| **1. Transport** STOMP ↔ Boxpro (LAN) | ✅ **reste sur l'agent** | seul rôle qui *doit* être sur le LAN du terrain |
| **2. Traduction** trames Bodet → commandes | ⚖️ **au choix** (agent ou serveur) — voir variantes | équilibre autonomie ↔ maintenabilité |
...
|----------|---------------------|-------|-----------|
| **Agent « dumb pipe »** | relaie les messages STOMP **bruts** vers KPI ; le **worker serveur traduit** | boîtier quasi immuable (jamais de MAJ métier) ; **une seule** version de la logique Bodet (serveur) | bufferise aussi le tic-tac des chronos (plusieurs messages/s) → buffer volumineux, dédoublonnage serveur |
| **Agent « semi-dumb »** *(recommandé)* | **filtre** les ticks de chrono (ne transmet que les **transitions** — ce que `wsMixin.js` fait déjà implicitement) et relaie les trames **brutes filtrées** ; la traduction Bodet vit **une seule fois côté serveur** (ACL) | boîtier quasi-firmware (le filtrage de ticks est stable, jamais de MAJ métier) ; buffer petit (transitions seules) ; **DRY** : une seule implémentation de la logique Bodet, versionnée avec le reste | le filtrage doit connaître la forme minimale des trames chrono (risque faible : format stable) |
| **Agent « smart »** | **traduit** (réutilise `wsMixin.js`) et POST des **commandes normalisées** (`goal`, `card`, `clock run/stop`) | autonome ; buffer local rejoue des **commandes propres** après coupure | **duplique la logique Bodet sur une flotte de boîtiers** à mettre à jour à distance (fleet) — contradiction directe avec l'objectif d'harmonisation |
...
> semi-dumb, qui conserve l'atout décisif du dumb pipe : **la traduction Bodet n'existe qu'à un
...
- **Reste sur le LAN** : pas besoin d'exposer la Boxpro à Internet → **pas de point dur réseau**.
...
|---------------------|--------------------------------|
| **Hardware Scoring** (Bodet via agent/relais, opt. 1/2/3) | goal/card/timer/period/score |
...
3. Règle d'**arbitrage — tranchée : source active exclusive** (pas de *last-write-wins* / co-pilotage,
   ...
   - **Ré-ancrage de la baseline** : la Bodet envoie des **scores cumulés** ; l'action but/annulation
...
5. **Divergence base ↔ Scorepad** : le Scorepad a son **propre état, incorrigible à distance**
   (hors `set-teams`). Si la base dit 3–2 et le Scorepad 4–2, il n'y a **pas de résolution
...
- La Boxpro émet le chrono en **continu** (plusieurs messages par seconde), le temps arrivant en
...
| Chrono | Source Boxpro | Précision cible | Aujourd'hui | Avec option 4 |
|--------|---------------|-----------------|-------------|---------------|
...
*seulement si* on confirme qu'une Boxpro peut émettre vers le serveur (à vérifier auprès de Bodet).
...
1. **Arbitrage multi-source** : source active **exclusive** (pas de last-write-wins). Commandes
   ...
   sur divergence base ↔ Scorepad. Détail : §7.4.
...
| **P3 — Agent local headless (semi-dumb)** | Boîtier filtrant (transitions seules), buffer offline avec **replay testé automatiquement**, auth machine, NTP + clé 4G au kit ; la traduction Bodet reste **serveur** (ACL unique). Supervision app4 lit la base. Retrait progressif de l'onglet WSM. | O1, O2 (plein) | ★★★☆☆ |
...
1. **Boxpro peut-elle émettre vers le serveur ?** (débloque/écarte l'option 1). — À vérifier Bodet.

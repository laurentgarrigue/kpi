...
| 1.2a | `scoring_live_clock` : arrêter le chrono sur un dixième non nul (ex. affichage `07:42.3`) | `elapsed_ms` est enregistré **au dixième** (`…300`) et non arrondi à la seconde (`…000`) ; idem `SHOTCLOCK` et `PENALTY`. Le contrat `/gameTimer` transporte désormais des secondes **fractionnaires** dans les deux sens (un client qui envoie des entiers reste valide) |
| 1.2b | Recharger la page chrono en marche | le temps restauré conserve le dixième (`started_at` est un `datetime(3)`, la dérive est compensée en sub-seconde) |
| 1.2c | Arrêter le chrono sur un dixième non nul (ex. `09:06.5`), **rouvrir le match dans un 2ᵉ onglet**, puis **relancer** depuis cet onglet | le chrono repart **exactement de 09:06.5** — pas de saut à `09:07.0` ni de valeur aléatoire au-dessus/en-dessous. (Régression corrigée le 2026-08-02 : easytimer réamorçait en secondes entières **et** écrasait le restant précis, si bien que le dixième était reperdu **à chaque** cycle arrêt/relance, jusqu'à +0,9 s offert par cycle) |
| 1.2d | Enchaîner **5 arrêts/relances** sur le même match, en notant `elapsed_ms` à chaque arrêt | la partie décimale reste **stable** (pas de dérive cumulative vers le haut) ; idem pour les **pénalités** (`kind='PENALTY'`), qui doivent conserver leur reliquat sub-seconde d'un cycle à l'autre |
| 1.2e | Pause inter-période (`kind='BREAK'`) | **exception assumée** : décompte à la seconde, purement indicatif — aucune précision au dixième attendue (spec §6.4) |
...
| 3.18 | Laisser le chrono principal atteindre **0** en fin de M1 | buzzer + décompte de **pause 3 min** affiché (1 min entre prolongations) ; buzzer en **fin de pause** ; « Terminer la pause » l'interrompt |
| 3.18a | Pendant la pause, **changer de période** (M1 → M2) | le décompte de pause **continue** — l'opérateur prépare la période suivante pendant que les équipes se reposent (corrigé le 2026-08-02 : il s'arrêtait à tort). Le **chronomètre de tir**, lui, repasse bien à `--` |
| 3.18b | Pendant la pause, **lancer le chrono principal** | la pause s'arrête **et disparaît** (seul événement qui la termine, avec sa fin naturelle et « Terminer la pause ») ; ligne `BREAK` supprimée de `scoring_live_clock` |
| 3.18c | Boutons **−10 / −1 / +1 / +10** du chrono principal | ils agissent dans le sens de l'**affichage** (temps restant) : `+1` **ajoute** une seconde au temps restant, `−1` l'enlève (inversés avant le 2026-08-02). Bornés : jamais sous `00:00.0` ni au-dessus de la durée de la période |
